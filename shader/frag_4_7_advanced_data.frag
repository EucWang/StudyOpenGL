...
void main(){
	...
	vec3 tex1 = vec3(texture(texture_diffuse1, texCoords));
	vec3 tex2 = vec3(texture(texture_specular1, texCoords));
	...
	result += calcPointLight(pointlight, norm, fragPos, viewDir, tex1, tex2);
	result += calcSpotLight(spotlight, norm, fragPos, viewDir, tex1, tex2);
